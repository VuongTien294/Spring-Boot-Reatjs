...
export default class SearchCoupon extends React.Component {
  ...
  loadCoupon = async () => {
    ...
    var raw = JSON.stringify(this.state.search);
    ...
    try {
      let response = await fetch("http://localhost:8080/api/admin/coupon/search", requestOptions)
  ...
  loadMore = () => {
    ...
    this.setState({ search }, this.loadCoupon)
  ...
  delete = (id) => {
    ...
    fetch("http://localhost:8080/api/admin/coupon/delete?id=" + id, requestOptions)
      .then(response => {
        if (response.ok) {
    ...
    return <>
      <div className="flex flex-wrap mt-4">
        <div className="w-full mb-12 px-4">
          <CardSearchCoupon dataProperties={this.state.response.data} deleteProperties={this.delete} />
